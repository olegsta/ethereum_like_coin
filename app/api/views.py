    ...
    if g.symbol == coin_symbol:
        try:
            ...
            if (transaction["to"] in list_accounts) or (
                ...
                logger.warning(
                    f"Addresses in {txid} is not related to any SHKeeper addresses. Checking {txid} as a smartcontract internal transaction"
                )
                ...
                logger.warning(
                    f"Regular {coin_symbol} transactions to our addresses in {block_num} block: {block_eth_tx_addrs}"
                )
                ...
                for acc_addr in list_accounts:
                    if acc_addr[2:].lower() in tx_input:
                        if acc_addr not in block_eth_tx_addrs:
                            ...
                            logger.warning(
                                f"Found internal transaction to {acc_addr} but skip it because there was already a regular {coin_symbol} transaction to {acc_addr} in {block_num} block"
                            )

                if len(related_internal_addr) > 0:
                    logger.warning(
                        f"Found internal transactions to {related_internal_addr}"
                    )
                else:
                ...
                for token in config["TOKENS"][config["CURRENT_NETWORK"]].keys():
                    token_addresses.append(
                        config["TOKENS"][config["CURRENT_NETWORK"]][token][
                            "contract_address"
                        ]
                    )
                ...
                for tr in block.transactions:
                    ...
                    if (
                        (len(tr_input) > 6)
                        and (tr["to"] not in token_addresses)
                        and tr["hash"].hex() != txid
                    ):
                        for addr in related_internal_addr:
                            if addr[2:].lower() in tr_input:
                                logger.warning(
                                    f"Found another internal transaction {tr['hash'].hex()} to our address {addr} in the same block, skip it!"
                                )
                                addresses_in_another_txs.append(addr)

                clear_addresses = set(related_internal_addr) - set(
                    addresses_in_another_txs
                )

                if len(clear_addresses) == 0:
                    logger.warning(
                        "No addresses are exclusively associated with the requested transaction in this block; return an empty list"
                    )

                for acc_addr in clear_addresses:
                    balance_before = Decimal(
                        w3.from_wei(
                            w3.eth.get_balance(acc_addr, block_num - 1), "ether"
                        )
                    )
                    balance_after = Decimal(
                        w3.from_wei(w3.eth.get_balance(acc_addr, block_num), "ether")
                    )
                    category = "receive"
                    ...
                    if amount > 0:  # skip 0 amount smartcontract tx
                        related_transactions.append(
                            [acc_addr, amount, confirmations, category]
                        )
            ...
            logger.warning({"status": "error", "msg": str(e)})
            return []
        ...
        try:
            ...
            if len(transactions_array) == 0:
                logger.warning(
                    f"There is not any token {g.symbol} transaction with transactionID {txid}"
                )
