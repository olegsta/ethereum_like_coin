...
class _Account:
    def __init__(self, address, account_type="regular"):
...
def test_wallet_in_scope_regular_by_store_id():
    wallet = _Wallet("0xinv", "regular", store_id=2)
    assert wallet_in_scope(wallet, store_id=2, scoped=True) is True
    assert wallet_in_scope(wallet, store_id=3, scoped=True) is False


def test_account_in_scope_follows_wallet_store_id():
    account = _Account("0xinv")
    wallets = {"0xinv": _Wallet("0xinv", "regular", store_id=2)}
    assert account_in_scope(
        account, store_id=2, scoped=True, wallets_by_address=wallets
    ) is True
    assert account_in_scope(
        account, store_id=3, scoped=True, wallets_by_address=wallets
    ) is False
...
def test_unset_row_store_id_is_not_store_one():
    ...
    account = _Account("0xinv")
    assert account_in_scope(
        account, store_id=1, scoped=True, wallets_by_address={}
    ) is False


def test_get_drain_destination_requires_wallet_store_id():
    ...
    with patch("app.services.fda.Wallets") as wallets:
        ...
        row.type = "regular"
        row.store_id = None
        wallets.query.filter_by.return_value.first.return_value = row
        try:
...
def test_get_drain_destination_uses_wallet_store_id():
    from unittest.mock import MagicMock, patch
