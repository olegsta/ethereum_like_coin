...
class TestRuntimeConfig:
    ...
    def test_get_contract_address_for_sepolia_token(self):
        ...
        assert address == CHAINS['ETH'].TOKENS['sepolia']['ETH-USDT']['contract_address']
    ...
    def test_get_min_token_transfer_threshold_uses_per_token_override(self, monkeypatch):
        monkeypatch.setitem(config_module.config, 'COIN_NETWORK', 'sepolia')
        tokens = config_module.config['TOKENS']
        tokens['sepolia']['ETH-USDT']['min_transfer_threshold'] = Decimal('2.5')
        try:
            assert config_module.get_min_token_transfer_threshold('ETH-USDT') == Decimal('2.5')
        finally:
